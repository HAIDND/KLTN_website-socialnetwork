...
import {
  adminDeleteGroup,
  adminGetGroup,
} from "~/services/AdminServices/adminGroupService";
import AdminSelect from "./AdminSelect";
import {
  adminDeleteFriendships,
  adminGetFriendShips,
} from "~/services/AdminServices/adminFriendService";
import {
  adminDeletePost,
  adminGetPost,
} from "~/services/AdminServices/adminPostService";
import {
  adminDeleteMessage,
  adminGetMessage,
} from "~/services/AdminServices/adminMessageService";
import {
  adminDeleteNotifi,
  adminGetNotifi,
} from "~/services/AdminServices/notifiService";
...
function AdminPage() {
  ...
  //realtime online ủe
  const [listUserOnline, setlistUserOnline] = useState(null);
  const useronlineAPI = {
    get: () => {
      socket.on("isUserActive", (data) => {
        console.log(data);
        setlistUserOnline(data);
        return <div>user online</div>;
      });
    }, // Lấy danh sách bài viết
    delete: adminDeletePost, // Xóa bài viết
  };
  useEffect(() => {
    socket.on("isUserActive", (onlineUsers) => {
      console.log(onlineUsers);
      console.log("onlineUsers");
      setlistUserOnline(onlineUsers);
    });
  }, [route]);
  ///columns---------------------------------------------------------------------------------------------------------
  const userColumns = [
  ...
  //Useronline  column
  const useronlineColumns = [
    { field: "userId.username", label: "User name" }, // Tên người đăng bài
    { field: "userId.email", label: "Email" }, // Email của người đăng
    { field: "ID user", label: "ID ủe" }, // Nội dung bài viết
    { field: "ID socket", label: "IDsocket " }, // Tổng số lượt thích
    { field: "Status ", label: "Trạng thái đăng nhập" }, // Tổng số bình luận
  ];
  ...
  useEffect(() => {
    switch (route) {
      ...
      case "post":
        ...
        break;
      case "useronline":
        setAPI(useronlineAPI);
        setColumns(useronlineColumns);
        break;
      default:
      ...
      <Grid item flex={8} sx={{ mt: 12, height: "100%", overflow: "auto" }}>
        ...
        {route === "notifi" && (
          <AdminTable
            ...
            reload={reload}
            api={notifiAPI}
            columns={notifiColumns}
          />
        )}
        {/* {route === "useronline" && useronlineAPI.get()} */}
        {/* Thêm dòng này */}
      </Grid>
    </Grid>
  );
